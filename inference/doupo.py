...
tf.flags.DEFINE_string('checkpoints_dir', os.path.abspath('./checkpoints/doupo/'), 'checkpoints save path.')
tf.flags.DEFINE_string('file_path', os.path.abspath('./dataset/data/doupo.txt'), 'file name of poems.')
    ...
    with tf.Session() as sess:
        ...
        try:
            for epoch in range(start_epoch, FLAGS.epochs):
                ...
                if epoch % 6 == 0:
                    saver.save(sess, './doupo_model/', global_step=epoch)
...
def gen_poem(begin_word):
    ...
    with tf.Session() as sess:
        ...
        checkpoint = tf.train.latest_checkpoint('./doupo_model/')
        saver.restore(sess, './doupo_model/-30')
...
def main(is_train):
    print('doupo.main:', is_train)
    if is_train:
        print('[INFO] train doupo fiction...')
        run_training()
    else:
        print('[INFO] write doupo fiction...')
